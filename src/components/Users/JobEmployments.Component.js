import React,{Fragment} from "react";
import { Link } from "react-router-dom"
import { Accordion, Card, Dropdown } from "react-bootstrap";
import menu from "./../../assets/img/menu.png"
const JobEmployments = (props) => {


	console.log("Job Employments", props)
	const jobs = props.empExperiences;
	console.log("Jobs", jobs)

	return (
		<Card>
			<Accordion.Toggle as={Card.Header} eventKey="4"   className="mb-2">
				<Link className="collapsed" to="/admin/users/profile"><h5 className="mb-0">
                    Job Employments
					<i className="fa fa-angle-down rotate-icon" />
				</h5></Link>
			</Accordion.Toggle>
			<Accordion.Collapse eventKey="4">
				<Fragment>
					<div className="profile-gap">
						{}
						<div
							id="collapseFive5"
							role="tabpanel"
							aria-labelledby="headingFive5"
							data-parent="#accordionEx"
						>
							<div className="card-header1 mb-2" role="tab">
								<h5 className="mb-0">
                                    Employment Info
									<button className="btn btn-default">Add</button>
								</h5>
							</div>
							<div className="container">
								<div className="row">
									<div className="col-md-12">
										<div className="row">
											<div className="col-md-12 bg-prt bg-white">
												{}
												{jobs.map((job,index)=> {
													return(
														<div className="card">
															{}
															<div
																id="collapseFive5"
																role="tabpanel"
																aria-labelledby="headingFive5"
																data-parent="#accordionEx"
															>
																<div className="card-body">
																	<div className="row">
																		<div className="col-md-5">
																			<table className="table table-sm accordionText">
																				<tbody>
																					<tr>
																						<td>Experienced Type:</td>
																						<td>{{...job.experienceType}.name}</td>
																					</tr>
																					{/* <tr>
																				<td>Employee Code:</td>
																				<td>{props.empCode}</td>
																			</tr> */}
																					{/* <tr>
																				<td>Last Designation:</td>
																				<td>{props.lastDesignation}</td>
																			</tr> */}
																					<tr>
																						<td>
                                                                                    Service Start Date:{" "}
																						</td>
																						<td>{job.startDate} </td>
																					</tr>
																					{/* <tr>
																				<td>Employment Type: </td>
																				<td>{{...props.employmentType}.name} </td>
																			</tr> */}
																					{/* <tr>
																				<td>
                                                                                    Total no. of promotions
                                              during the stay:{" "}
																				</td>
																				<td>{props.totalYearsOfStay}</td>
																			</tr> */}
																					{/* <tr>
																				<td>Joining CTC: </td>
																				<td>{props.joiningCTC}</td>
																			</tr> */}
																					{/* <tr>
																				<td>
                                                                                    Average % growth in CTC
                                              during the stay:{" "}
																				</td>
																				<td>{props.averageGrowth}</td>
																			</tr>
																			<tr>
																				<td>
                                                                                    Major Milestones achieved
                                              during stay:{" "}
																				</td>
																				<td>{props.majorMilestones}</td>
																			</tr> */}
																				</tbody>
																			</table>
																		</div>
																		<div className="col-md-6">
																			<table className="table table-sm accordionText">
																				<tbody>
																					<tr>
																						<td>Name of Company:</td>
																						<td>
																							{job.companyName}
																						</td>
																					</tr>
																					{/* <tr>
																				<td>
                                                                                    Place of Posting/Location:
																				</td>
																				<td>{props.placeOfPostingLocation}</td>
																			</tr>
																			<tr>
																				<td>First Designation:</td>
																				<td>{props.firstDesignation}</td>
																			</tr> */}
																					<tr>
																						<td>Service End Date:</td>
																						<td>{job.endDate}</td>
																					</tr>
																					{/* <tr>
																				<td>
                                                                                    Total no. of years of
                                                                                    stay:
																				</td>
																				<td>{props.totalYearsOfStay}</td>
																			</tr>
																			<tr>
																				<td>
                                                                                    Major Responsibilities
                                                                                    handled:
																				</td>
																				<td>{props.majorResponsibilities}</td>
																			</tr>
																			<tr>
																				<td>
                                                                                    Last Withdrawn CTC:{" "}
																				</td>
																				<td>{props.lastWithdrawnCTC} </td>
																			</tr>
																			<tr>
																				<td>Reason of Leaving: </td>
																				<td>{props.reasonOfLeaving}</td>
																			</tr>
																			<tr>
																				<td>
                                                                                    Major Milestones achieved
                                              during stay:{" "}
																				</td>
																				<td>
																					{props.majorMilestones}
																				</td>
																			</tr> */}
																				</tbody>
																			</table>
																		</div>
																		<div className="col-md-1 btn-section text-right">
																			<div className="btn-group">
																				<Dropdown>
																					<Dropdown.Toggle className="dropdown-toggle"
																						variant="nav-link text-primary"
																						id="dropdown-basic">
																						<img src={menu} alt="menu" />
																					</Dropdown.Toggle>
																					<Dropdown.Menu className="dropdown-menu">
																						<Dropdown.Item href="#" className="dropdown-item">Edit</Dropdown.Item>
																						<Dropdown.Item href="#" className="dropdown-item">History</Dropdown.Item>
																					</Dropdown.Menu>
																				</Dropdown>
																			</div>
																		</div>
																	</div>
																</div>
															</div>
														</div>
													)})}
											</div>
											{}
										</div>
									</div>
								</div>
							</div>
						</div>
					</div>

				</Fragment>
			</Accordion.Collapse>
		</Card>

	)
}

export default JobEmployments;
